...
import static kg.softech.controller.ProductsController.getProductCount;
...
public class CategoriesController {
    ...
    public String getCategory(@PathVariable String categoryName,
                              Model model,
    ...
    public String getSubCategory(@PathVariable String categoryName,
        ...
        for (Categories category : categories) {
            for (SubCategories subCategory : category.getSubCategory()) {
                if (categoryName.equals(category.getTranslit()) && subCategoryName.equals(subCategory.getTranslit())) {
                    ...
                    model.addAttribute("title", GlobalVar.storeName + " - " + category.getName() + " - " + subCategory.getName());
                    model.addAttribute("category", categoryName);
                    model.addAttribute("categoryMeta", category.getDescription());
    ...
    public static List<Product> getProductsByCategory(int localCatId) {
        ...
        try {
            ...
            URL url = new URL(GlobalVar.mainURL + "getProductsByCategory?storeId=" + GlobalVar.storeId + "&limit=12&" + "localCatId=" + localCatId);
            ...
            for (int i = 0; i < categoriesArray.length(); i++) {
                products.add(new Gson().fromJson(categoriesArray.getJSONObject(i).toString(), Product.class));
            }
