...
import kg.softech.model.UserForSignin;
import kg.softech.model.UserWithAllInfo;
...
import static kg.softech.controller.CategoriesController.getProductsByCategory;
...
//            }
    ...
    public List<Product> newIncomeProducts() {
        ...
        try {
            ...
            StringBuilder builder = new StringBuilder();
            Gson gson = new GsonBuilder().create();
            URL url = new URL(GlobalVar.mainURL + "getNewIncomeProducts?limit=10&storeId=" + GlobalVar.storeId);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
